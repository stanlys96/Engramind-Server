...
interface QuickRoleplayJobResult {
  jobStatus: string;
  message: string;
  processedId: string;
}

@Processor(CREATE_QUICK_ROLEPLAY_QUEUE)
export class CreateQuickRoleplayProcessor extends WorkerHost {
  async process(
    job: Job<CreateQuickRoleplayDTO, QuickRoleplayJobResult, string>,
  ): Promise<QuickRoleplayJobResult> {
    const { id, data } = job;
    try {
      ...
      try {
        ...
        const apiResponse: QuickRoleplayJobResult = {
          jobStatus: 'success',
